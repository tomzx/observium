...
#         </tr>');
  ...
  $background = get_percentage_colours($average_percent);
    ...
    echo('<table class="table table-condensed-more table-striped table-bordered">');
    echo('<tr>
           <td><span class="entity">'.overlib_link($link, $text_descr, $overlib_content).'</span><span class="pull-right small">(*'.$numprocs.')</span></td>
           <td width=90>'.overlib_link($link, $minigraph, $overlib_content).'</td>
           <td width=200>'.overlib_link($link, print_percentage_bar (200, 20, $average_percent, NULL, "ffffff", $background['left'], $percent . "%", "ffffff", $background['right']), $overlib_content).'
           </a></td>
         </tr>');
