...
function api_device_permitted($device_id) {
  ...
  if ($vars['user']['level'] >= 10) {
    ...
    api_show_debug("Cheking permition for device", $vars['device']);
    $row = dbFetchRow("SELECT * FROM `devices_perms` WHERE `user_id` = ? AND `device_id`= ? LIMIT 1", array($vars['user']['id'], $vars['device']));
    if (is_array($row)) {
      $res  = true;
    }
  }
