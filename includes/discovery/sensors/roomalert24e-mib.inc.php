...
# Silly AVTECH made all different MIBs per product - doh.

if ($device['os'] == 'roomalert')
{
  ...
  # We will detect the divisor based on whether the measured humidity is > 100. This will fail on 4-digit humidity < 1% ;-)
  $divisor = 1; # Start at 1 for 2 digits setting.
  ...
  if (is_array($oids))
  {
    foreach ($oids as $index => $entry)
    {
      ...
      $current = $entry['internal-humidity'];
      if ($current > 100) { $divisor = 100; $current /= 100; }
      discover_sensor($valid['sensor'], 'humidity', $device, $oid, "internal-humidity.$index", 'roomalert', $descr, $divisor, 1, NULL, NULL, NULL, NULL, $current);
    }
  }
  ...
  if (is_array($oids))
  {
    foreach ($oids as $index => $entry)
    {
      ...
      $current = $entry['internal-tempc'] / $divisor;
      discover_sensor($valid['sensor'], 'temperature', $device, $oid, "internal-tempc.$index", 'roomalert', $descr, $divisor, 1, NULL, NULL, NULL, NULL, $current);
    }
  }
  ...
  for ($i = 1;$i <= 6;$i++)
  {
    if (isset($oids[$index]["digital-sen$i-1"]))
    {
      ...
      if (!isset($oids[$index]["digital-sen$i-3"]))
      {
        ...
        $current = $oids[$index]["digital-sen$i-1"] / $divisor;
        discover_sensor($valid['sensor'], 'temperature', $device, $oid, "digital-sen$i-1.$index", 'roomalert', $descr, $divisor, 1, NULL, NULL, NULL, NULL, $current);
      }
      elseif (isset($oids[$index]["digital-sen$i-5"]))
      {
        ...
        $current = $oids[$index]["digital-sen$i-5"] / $divisor;
        discover_sensor($valid['sensor'], 'temperature', $device, $oid, "digital-sen$i-5.$index", 'roomalert', $descr, $divisor, 1, NULL, NULL, NULL, NULL, $current);

        $descr = "Channel $i: Humidity";
        $oid = ".1.3.6.1.4.1.20916.1.5.1.2.$i.3.".$index;
        $current = $oids[$index]["digital-sen$i-3"] / $divisor;
        discover_sensor($valid['sensor'], 'humidity', $device, $oid, "digital-sen$i-3.$index", 'roomalert', $descr, $divisor, 1, NULL, NULL, NULL, NULL, $current);
      } else {
        # Power sensor
        $descr = "Channel $i: Current";
        $oid = ".1.3.6.1.4.1.20916.1.5.1.2.$i.1.".$index;
        $current = $oids[$index]["digital-sen$i-1"] / $divisor;
        discover_sensor($valid['sensor'], 'current', $device, $oid, "digital-sen$i-1.$index", 'roomalert', $descr, $divisor, 1, NULL, NULL, NULL, NULL, $current);

        $descr = "Channel $i: Power";
        $oid = ".1.3.6.1.4.1.20916.1.5.1.2.$i.2.".$index;
        $current = $oids[$index]["digital-sen$i-2"] / $divisor;
        discover_sensor($valid['sensor'], 'power', $device, $oid, "digital-sen$i-2.$index", 'roomalert', $descr, $divisor, 1, NULL, NULL, NULL, NULL, $current);

        $descr = "Channel $i: Voltage";
        $oid = ".1.3.6.1.4.1.20916.1.5.1.2.$i.3.".$index;
        $current = $oids[$index]["digital-sen$i-3"] / $divisor;
        discover_sensor($valid['sensor'], 'voltage', $device, $oid, "digital-sen$i-3.$index", 'roomalert', $descr, $divisor, 1, NULL, NULL, NULL, NULL, $current);

        $descr = "Channel $i: Reference voltage";
        $oid = ".1.3.6.1.4.1.20916.1.5.1.2.$i.4.".$index;
        $current = $oids[$index]["digital-sen$i-4"] / $divisor;
        discover_sensor($valid['sensor'], 'voltage', $device, $oid, "digital-sen$i-4.$index", 'roomalert', $descr, $divisor, 1, NULL, NULL, NULL, NULL, $current);
      }
    }
  }
}
