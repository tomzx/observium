...
if ($device['os'] == "papouch")
{
  ...
  {
    if (snmp_get($device, "SNMPv2-SMI::enterprises.18248.20.1.3.1.1.1.1", "-Oqv"))
    {
      $low_limit = snmp_get($device, "SNMPv2-SMI::enterprises.18248.20.1.3.1.1.2.1", "-Oqv") / 10;
  ...
  $temperature = snmp_get($device, "SNMPv2-SMI::enterprises.18248.20.1.2.1.1.2.3", "-Oqv") / 10;

  if (is_numeric($temperature) && $temperature > "0")
  {
    if (snmp_get($device, "SNMPv2-SMI::enterprises.18248.20.1.3.1.1.1.3", "-Oqv"))
    {
      $low_limit = snmp_get($device, "SNMPv2-SMI::enterprises.18248.20.1.3.1.1.2.3", "-Oqv") / 10;
      $high_limit = NULL; # The MIB is invalid and I can't find the max value in snmpwalk :[ *sigh*
